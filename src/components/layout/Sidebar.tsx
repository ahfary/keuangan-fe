...
const waliSantriMenu = (santriId: string | null): NavItem[] => [
  ...
  {
    href: "/dashboard/walsan/transfer",
    icon: DollarSign,
  ...
  },
  {
    ...
    submenu: santriId
      ? [
          {
            href: `/dashboard/walsan/santri/${santriId}#jajan`,
            icon: Circle,
            label: "Riwayat Jajan",
          },
          {
            href: `/dashboard/walsan/santri/${santriId}#hutang`,
            icon: Circle,
            label: "Riwayat Hutang",
          },
          {
            href: `/dashboard/walsan/santri/${santriId}#tunai`,
            icon: Circle,
            label: "Riwayat Tarik Tunai",
          },
        ]
      : [],
  },
...
const CollapsibleNavLink = ({ item }: { item: NavItem }) => {
  ...
  const isParentActive =
    item.submenu?.some((sub) => pathname.startsWith(sub.href.split("#")[0])) ??
    false;
      ...
      <div className="flex flex-col">
        <div className="flex items-center">
          <Image
            ...
            className={"mb-4"}
          />
          <div className="text-2xl font-bold text-indigo-600 mb-8 pt-6 ml-2">
            SakuSantri
          </div>
        </div>
